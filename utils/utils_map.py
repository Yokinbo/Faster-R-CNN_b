...
def preprocess_gt(gt_path, class_names, image_id_map=None, image_sizes=None):
    image_ids   = sorted(os.listdir(gt_path))
    if image_id_map is None:
        image_id_map = {
            os.path.splitext(name)[0]: index + 1
    ...
    for i, image_id in enumerate(image_ids):
        ...
        numeric_image_id = image_id_map[image_id]
        image['file_name'] = image_id + '.jpg'
        size = (image_sizes or {}).get(image_id, {})
        image['width']     = int(size.get('width', 1))
        image['height']    = int(size.get('height', 1))
        #-----------------------------------------------------------------#
        #   感谢 多学学英语吧 的提醒
        #   解决了'Results do not correspond to current coco set'问题
        #-----------------------------------------------------------------#
        image['id']        = numeric_image_id
        ...
        for line in lines_list:
            ...
            bbox    = [left, top, right - left, bottom - top, difficult, numeric_image_id, cls_id, (right - left) * (bottom - top)]
...
def preprocess_dr(dr_path, class_names, image_id_map=None):
    image_ids = sorted(os.listdir(dr_path))
    if image_id_map is None:
        image_id_map = {
            os.path.splitext(name)[0]: index + 1
            for index, name in enumerate(image_ids)
    ...
    for image_id in image_ids:
        ...
        for line in lines_list:
            ...
            if image_id not in image_id_map:
                continue
            result["image_id"]      = image_id_map[image_id]
            if class_name not in class_names:
                continue
...
def get_coco_map(class_names, path):
    ...
    image_id_map = {
        os.path.splitext(name)[0]: index + 1
        for index, name in enumerate(sorted(os.listdir(GT_PATH)))
    }
    image_sizes_path = os.path.join(path, 'image_sizes.json')
    image_sizes = {}
    if os.path.isfile(image_sizes_path):
        with open(image_sizes_path, encoding='utf-8') as f:
            image_sizes = json.load(f)

    with open(GT_JSON_PATH, "w") as f:
        results_gt  = preprocess_gt(GT_PATH, class_names, image_id_map, image_sizes)
        json.dump(results_gt, f, indent=4)

    with open(DR_JSON_PATH, "w") as f:
        results_dr  = preprocess_dr(DR_PATH, class_names, image_id_map)
